	...
	for (var i = 0; i < auc[0].length; i++) { //goes through auctions
		...
		try {
			...
			if (itemName[0].search(itemSearch) !== -1) {
				...
				addItemtoArray(itemData, capFirstLets(itemName[0]), item.starting_bid, item.auctioneer, item.bin, item.tier, capFirstLets(itemName[0]), item.item_lore, itemID, CSN);
